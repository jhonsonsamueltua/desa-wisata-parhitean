...
<html lang="en">
	<head>
		...
		<title>Daftar Transaksi | Parhitean</title>
...
$userid = $_SESSION['id'];
$query = "SELECT * FROM transaksi WHERE status_transaksi = 1 OR status_transaksi = 3";
$gquery = mysqli_query($conn,$query);
while($trans = mysqli_fetch_array($gquery))
{
		$idsts = $trans['status_transaksi'];
		$qgetsts = "SELECT * FROM status_transaksi WHERE id_status = $idsts";
		$gsts = mysqli_query($conn,$qgetsts);
		$status = mysqli_fetch_array($gsts);
  ...
  ?>
            <tbody>
              <tr>
                ...
                <td><?=$status['nama']?> <font color="red">*</font> </td>
                <td colspan="2">
                  <a href="view_transaksi.php?id=<?=$trans['id_transaksi']?>"><button class="btn btn-info"><i class="fa fa-eye"></i> View</button></a>
              </tr>
            </tbody>
